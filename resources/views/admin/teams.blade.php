...
@section('nav-items')
    ...
    <li class="nav-item">
        <a class="nav-link active" href="/apanel/teams">Teams </a>
    </li>
    <li class="nav-item">
    ...
    <div class="table-responsive">
        <table class="table table-striped mb-5">
            ...
            <tr>
                ...
                <th>City</th>
                <th>Image</th>
                <th><a href="/teams/create" class="btn btn-primary float-right">Add</a></th>
            </tr>
            </thead>
            <tbody>
            @foreach($data as $single_data)
                <tr>
                    ...
                    <td><a href="/teams/{{$single_data->id}}">{{$single_data->name}}</a></td>
                    <td>{{$single_data->city}}</td>
                    <td><img id="img" src="{{$single_data->image}}"></td>
                    <td>
                        <form action="/teams/{{$single_data->id}}" method="POST">
                            @csrf
                            <input type="hidden" name="_method" value="delete" />
                            <button class="btn btn-sm btn-danger float-right ml-2">Delete</button>
                        </form>
                        <a href="/teams/edit/{{$single_data->id}}" class="btn btn-sm btn-primary float-right">Edit</a>
                    </td>
                </tr>
            @endforeach
            </tbody>
